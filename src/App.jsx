...
const projects = [
  ...
  { title: "Real-time Chat App", description: "A responsive chat app featuring real-time messaging and user authentication.", link: "https://github.com/Sachin-YN/chat-app" },
];
    ...
    <nav className="fixed top-0 left-0 h-full w-20 bg-secondary-bg bg-opacity-50 backdrop-blur-md flex flex-col items-center py-8 z-50">
      {navItems.map(item => (
        <a
          key={item.id}
          href={`#${item.id}`}
          className="mb-8 text-accent text-opacity-80 hover:text-opacity-100 hover:drop-shadow-[0_0_8px_rgba(0,209,178,0.8)] transition duration-300 text-2xl"
          aria-label={item.id}
        >
          {item.icon}
        </a>
...
function SectionHeading({ icon: Icon, text }) {
  return (
    <div className="mb-8 flex items-center justify-center md:justify-start">
      <Icon className="text-accent text-3xl mr-3" />
      <h2 className="text-4xl font-bold text-white relative after:absolute after:bottom-0 after:left-0 after:w-16 after:h-1 after:bg-accent after:rounded-full">
        {text}
      </h2>
    </div>
  );
...
function About() {
  return (
    <section id="about" className="min-h-screen flex items-center justify-center bg-primary-bg snap-start px-6 md:px-0">
      <motion.div initial={{ x: -40, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} transition={{ duration: 0.8 }} className="max-w-3xl bg-secondary-bg bg-opacity-60 backdrop-blur-lg border border-accent rounded-2xl p-10 shadow-card text-center md:text-left">
        <SectionHeading icon={AiOutlineUser} text="About Me" />
...
function Projects() {
  return (
    <section id="projects" className="min-h-screen bg-primary-bg snap-start px-6 md:px-0 py-16">
      <SectionHeading icon={AiOutlineFolderOpen} text="My Projects" />
...
function Contact() {
  return (
    <section id="contact" className="min-h-screen flex items-center justify-center bg-primary-bg snap-start px-6 md:px-0">
      <motion.div initial={{ y: 40, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ duration: 0.8 }} className="relative bg-secondary-bg bg-opacity-60 backdrop-blur-lg border border-accent rounded-2xl p-10 shadow-card text-center max-w-2xl">
        <SectionHeading icon={AiOutlineMail} text="Let’s Connect" />
